...
def Wsd_Data_Install_Monthly(code, fields, enddate, forwardmonths):
    temp_data = w.wsd(code, fields, "ED-%sM"%forwardmonths, enddate, "Period=M")
...
def Index_Performance(code, enddate, forwardmonths):
    wsd_data = Wsd_Data_Install_Monthly(code, ["pct_chg", "swing", "amt", "free_turn"], enddate, forwardmonths)
...
def Bondindex_Performance(code, enddate, forwardmonths):
    wsd_data = Wsd_Data_Install_Monthly(code, ["pct_chg", "swing", "amt"], enddate, forwardmonths)
    today_data = list(wsd_data.iloc[-1])
    upper_quantile = list(wsd_data.iloc[:-1].quantile(0.85))
    lower_quantile = list(wsd_data.iloc[:-1].quantile(0.15))
...
def Foreignindex_Performance(code, enddate, forwardmonths):
    wsd_data = Wsd_Data_Install_Monthly(code, ["pct_chg", "swing"], enddate, forwardmonths)
    today_data = list(wsd_data.iloc[-1])
    upper_quantile = list(wsd_data.iloc[:-1].quantile(0.85))
    lower_quantile = list(wsd_data.iloc[:-1].quantile(0.15))
...
def Interest_Performance(code, enddate, forwardmonths):
    wsd_data = Wsd_Data_Install_Monthly(code, ["close3"], enddate, forwardmonths)
    today_data = list(wsd_data.iloc[-1])
    upper_quantile = list(wsd_data.iloc[:-1].quantile(0.85))
    lower_quantile = list(wsd_data.iloc[:-1].quantile(0.15))
...
forwardmonths = 37 #数据应包含上周数据及之前52周
enddate = "2017-01-31"
...
for each_index in domestic_list:
    temp_result, all_data = Index_Performance(each_index, enddate, forwardmonths)
...
for each_index in commodity_list:
    temp_result, all_data = Foreignindex_Performance(each_index, enddate, forwardmonths)
...
col_no = 1
temp_result, all_data = Interest_Performance("VIX.GI", enddate, forwardmonths)
...
for each_index in interest_list:
    temp_result, all_data = Interest_Performance(each_index, enddate, forwardmonths)
...
for each_index in bond_list:
    temp_result, all_data = Bondindex_Performance(each_index, enddate, forwardmonths)
